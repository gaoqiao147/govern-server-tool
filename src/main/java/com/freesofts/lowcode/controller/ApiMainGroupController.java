 ...
 */
@RestController
@RequestMapping("/apiMainGroupDO")
public class ApiMainGroupController {
    @Resource
    ApiMainGroupService apiMainGroupService;

    @PostMapping("/save-a-data")
    @LogAnnotation(module = "新增")
    @ApiOperation(value = "新增一条数据源信息", httpMethod = "POST")
    public BizResponseData<?> saveMainGroup(@ApiParam(value = "apiMainGroup") @RequestBody ApiMainGroup apiMainGroup) {
        int result = apiMainGroupService.saveMainGroup(apiMainGroup);
    ...
    public BizResponseData<?> saveMainGroup(@ApiParam(value = "List<ApiMainGroup>") @RequestBody List<ApiMainGroup> list) {
        int result = apiMainGroupService.saveMainGroups(list);
    ...
    public BizResponseData<?> getAllData() {
        List list = apiMainGroupService.getAll();
    ...
    public BizResponseData<?> updateData(
            @ApiParam(value = "数据id") @PathVariable("id") String apiId,
            @ApiParam(value = "数据对象") @RequestBody @Validated ApiMainGroup apiMainGroup
    ) {
        int result = apiMainGroupService.updateById(apiId, apiMainGroup);
        return IntegerBizResult.builder().value(result).build().beEqualTo(1);
    }
    ...
    public BizResponseData<?> deleteDataById(@ApiParam(value = "参数信息") String apiId) {
        int result = apiMainGroupService.deleteById(apiId);
        return IntegerBizResult.builder().value(result).build().greaterThan(-1);
    }
    ...
    public BizResponseData<?> deleteDataByIds(@ApiParam("参数信息") List<String> ids) {
        int result = apiMainGroupService.deleteByIds(ids);
        return IntegerBizResult.builder().value(result).build().greaterThan(-1);
    }
}
